  ...
  <div v-if="isOpen" class="fixed inset-0 bg-black/60 z-[100] flex items-center justify-center p-4 backdrop-blur-sm transition-all">
    <div class="bg-mulled-wine-600 border border-mulled-wine-500 w-full max-w-sm rounded-2xl shadow-2xl animate-scale-in">
      ...
      <div class="p-4 pt-2 flex gap-3">
        <button 
          @click="handleCancel" 
          class="flex-1 py-3 px-4 rounded-xl bg-notify-error/10 text-notify-error hover:bg-notify-error hover:text-white transition-colors active:scale-95 font-semibold"
        >
          {{ cancelText }}
        </button>
        <button 
          @click="handleConfirm" 
          class="flex-1 py-3 px-4 rounded-xl font-semibold transition-colors active:scale-95"
          :class="danger ? 'bg-notify-error text-white hover:bg-notify-error/80' : 'bg-mulled-wine-500 text-white hover:bg-mulled-wine-400'"
        >
          {{ confirmText }}
        </button>
      </div>
